...
def my_model(ssc, monitoring_dir, window_duration, sliding_duration, time_step_interval):
    ...
    winflatDStream = windowDStream.map(process_line)

    filteredDStream = flatDStream.filter( lambda x: (int(x[0]) == 0 and int(x[5])==0) )
    winfilteredDStream = winflatDStream.filter( lambda x: (int(x[0]) == 0 and int(x[5])==0) )
    
   
    mappedDStream = filteredDStream.map(lambda x:(x[1],int(x[5])+1))
    winmappedDStream = winfilteredDStream.map(lambda x:(x[1],int(x[5])+1))
   
    reducedDStream = mappedDStream.reduceByKey((lambda x, y: x + y))
    winreducedDStream = winmappedDStream.reduceByKey((lambda x, y: x + y))
    
    updatedDStream = reducedDStream.updateStateByKey(upd)
   
   
    
    sortedDStream = updatedDStream.transform( lambda rdd: rdd.sortBy(lambda x: x[1],False) )
    winsortedDStream = winreducedDStream.transform( lambda rdd: rdd.sortBy(lambda x: x[1],False) )
   

    sortedDStream.pprint()
    winsortedDStream.pprint()
