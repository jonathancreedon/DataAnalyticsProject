...
def my_main(sc, my_dataset_dir):
    ...
    filteredRDD = allRDD.filter( lambda x: (int(x[0]) == 0 and int(x[5])==0) )
    
   
    mappedRDD=filteredRDD.map(lambda x:(x[1],int(x[5])+1))
   
    reducedRDD = mappedRDD.reduceByKey((lambda x, y: x + y))
   
    sortedRDD = reducedRDD.sortBy(lambda x: x[1],False)
...
if __name__ == '__main__':
    ...
    my_local_path = "F:\CIT - Year 4 - Semester 2\Big Data & Analytics"
    #"/home/nacho/CIT/Tools/MyCode/Spark/"
    my_databricks_path = "/"
